defmodule ArtsyNeighborWeb.VendorLive.Dashboard do
    ...
    >
      <div class="space-y-10">
        ...
        <div class="flex items-center justify-between">
          ...
          <.link
            navigate={~p"/vendor/profile/edit" <> "?" <> create_return_to_params()}
        ...
        <div class="card card-side bg-base-200 shadow-sm">
          ...
          <div class="card-body">
            ...
            <div
              :if={@confirming_status}
              class="rounded-lg border border-warning bg-warning/10 p-4 space-y-3"
            >
            ...
            <div class="card-actions mt-2">
              <.link
                navigate={~p"/artists/#{@artist.id}" <> "?" <> URI.encode_query(return_to: "/vendor", return_label: "Artist Dashboard", preview: "1")}
                class="btn btn-ghost btn-xs"
              >
          ...
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex gap-2">
              <div
                ...
                data-tip="Create a collection of products — useful for when you have many diverse works"
              >
                  ...
                  <tbody>
                    <tr :if={collection.products == []}>
                      <td colspan="5" class="text-center text-base-content/50 py-6">
                        No products yet.
                        <.link navigate={~p"/vendor/products/new"} class="link link-primary ml-1">
                          Add one
                        </.link>
                      </td>
                    </tr>
                    <tr :for={{product, product_index} <- Enum.with_index(collection.products)}>
                      ...
                      <td>
                        <div class="flex gap-2 justify-end">
                          <.link
                            navigate={~p"/products/#{product.id}" <> "?" <> create_return_to_params()}
                            class="btn btn-ghost btn-xs"
                          >
                            View
                          </.link>
  ...
  def handle_event("move_product_up", %{"productid" => pid, "collectionid" => cid}, socket) do
    ...
    if index && index > 0 do
      ...
      {:noreply,
       assign(socket, collections: Products.list_collections_for_artist(socket.assigns.artist.id))}
    else
      {:noreply, socket}
    end
  end
  ...
  def handle_event("move_collection_up", %{"collectionid" => id}, socket) do
    ...
    if index && index > 0 do
      Products.swap_collection_positions(
        Enum.at(collections, index),
        Enum.at(collections, index - 1)
      )
