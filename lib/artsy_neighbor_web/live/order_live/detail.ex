        ...
        <div class="bg-base-200 rounded-2xl p-6 mt-4 flex flex-col gap-6">
          ...
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <.link
                navigate={
                  ~p"/flag/vendor/#{@order.artist_id}?#{[return_to: ~p"/orders/#{@order.id}", return_label: "Order details"]}"
                }
              ...
              <li :for={item <- @order.items} class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <.link
                    ...
                    navigate={
                      ~p"/flag/product/#{item.product_id}?#{[return_to: ~p"/orders/#{@order.id}", return_label: "Order details"]}"
                    }
                    class="text-xs text-base-content/40 underline"
                  >
                    Report this listing
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              <%= cond do %>
                <% is_nil(@vendor_review) and @days_left > 0 -> %>
                  <.link
                    navigate={~p"/orders/#{@order.id}/review/vendor"}
                  ...
                  <div class="flex flex-col items-end gap-1 shrink-0">
                    ...
                    <% else %>
                      <span class="text-xs text-base-content/40">
                        Reviews cannot be edited or deleted after {Reviews.edit_window_days()} days
                      </span>
                    <% end %>
                  </div>
              <% end %>
            ...
            <% else %>
              <%= if @received_review do %>
                <div class="flex flex-col gap-1">
                  ...
                  <p
                    :if={@received_review.body && @received_review.body != ""}
              ...
              <% else %>
                <p class="text-sm text-base-content/40 italic">
                  The vendor has not left a review yet.
                </p>
              <% end %>
            <% end %>
          </div>
