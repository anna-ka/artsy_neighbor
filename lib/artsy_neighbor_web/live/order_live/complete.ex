        ...
        <div class="bg-base-200 rounded-xl p-6 flex flex-col gap-6">
          <div>
            ...
            <p class="text-sm text-base-content/60">
              Confirm that you have received the following items from <span class="font-medium">{@order.artist_name}</span>.
            </p>
          </div>
          ...
          <ul class="flex flex-col gap-3">
            <%= for item <- @order.items do %>
              <li class="flex items-center gap-3">
                ...
                <span :if={item.quantity > 1} class="text-sm text-base-content/60">
                  ×{item.quantity}
                </span>
              </li>
            <% end %>
